    ...
    (testing "Datetime formatting"
      (let [date-col {:semantic_type :type/CreationTimestamp, :effective_type :type/Temporal}]
        (testing "date-style"
          ...
          (is (= "yyyy-mm-dd, h:mm am/pm"          (format-string {::mb.viz/date-style "YYYY-MM-DD"} date-col)))
          (is (= "yyyy.mm.dd, h:mm am/pm"          (format-string {::mb.viz/date-style "YYYY.MM.DD"} date-col)))
          (is (= "yyyy年m月d日, h:mm am/pm"        (format-string {::mb.viz/date-style "YYYY年M月D日"} date-col)))
      ...
      (let [date-col {:semantic_type :type/CreationTimestamp, :effective_type :type/Temporal}]
        (testing ":unit values on temporal breakout fields"
          (let [month-col (assoc date-col :unit :month)
            ...
            (is (= "yyyy-mm"    (format-string {::mb.viz/date-style "YYYY-MM-DD"} month-col)))
            (is (= "yyyy.mm"    (format-string {::mb.viz/date-style "YYYY.MM.DD"} month-col)))
            (is (= "yyyy年m月"  (format-string {::mb.viz/date-style "YYYY年M月D日"} month-col)))
