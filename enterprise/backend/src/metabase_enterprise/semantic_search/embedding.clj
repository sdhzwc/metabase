...
(declare configured-embedder-endpoint)
...
(defn embedder-circuit-state
  "Return the configured embedder circuit's state: `:closed`, `:open`, or `:half-open`.
  The optional `endpoint` arity inspects another endpoint's isolated circuit."
  ([]
   (embedder-circuit-state (configured-embedder-endpoint)))
  ([endpoint]
...
(defn- configured-embedder-endpoint
  []
  (case (:provider (get-configured-model))
    "ollama" ollama-embeddings-endpoint
    "ai-service" (first (embedding-service-resolve-config!))
    "openai" (first (openai-resolve-config!))))
