...
(def ^:private repair-count-max-age-seconds (* 3 60 60))
...
(defn- fresh-repair-count?
  [n snapshot-at]
  (and (some? n)
       snapshot-at
       (<= (t/time-between (t/instant snapshot-at) (t/instant) :seconds)
           repair-count-max-age-seconds)))

(defn- semantic-coverage []
  (when-let [{:keys [pgvector state]} (active-index)]
    (let [{:keys [repair_document_count repair_snapshot_at]} (:metadata-row state)]
      (when (fresh-repair-count? repair_document_count repair_snapshot_at)
        (let [table   (semantic.util/quote-table (-> state :index :table-name))
              ...
              ;; Only live, expected rows contribute to coverage. Orphans are reported separately and must
              ;; not conceal missing documents by inflating the numerator.
              indexed (row-count pgvector
                                 [(format (str "SELECT count(*) AS n FROM %s i "
                                          ...
                                          table gate)])]
          ;; Repair deduplicates the canonical document stream at the index's `(model, model_id)` grain.
          ;; Until that shared snapshot exists, coverage is unknown rather than a misleading 100% for 0/0.
          (search.index-health/coverage-result indexed repair_document_count))))))
    ...
    (let [index-metadata (semantic.env/get-index-metadata)
          ...
          dlq-row        (if (semantic.dlq/dlq-table-exists? pgvector index-metadata id)
                           (let [table (semantic.util/quote-table
                             ...
                             (scalar-row pgvector
                                         [(format "SELECT count(*) AS pending,
                                                          EXTRACT(EPOCH FROM (now() - min(d.error_gated_at))) AS age
                                                   FROM %s d
          ...
          age            (when (or (:age gate-row) (:age dlq-row))
                           (max (double (or (:age gate-row) 0))
                                (double (or (:age dlq-row) 0))))
          detail         (cond
...
(defn- semantic-garbage
  ...
  []
  (when-let [{:keys [state]} (active-index)]
    (let [{:keys [repair_orphan_count repair_snapshot_at]} (:metadata-row state)]
      (when (fresh-repair-count? repair_orphan_count repair_snapshot_at)
        (search.index-health/garbage-result
         repair_orphan_count garbage-warn-count garbage-critical-count)))))

(def ^:private semantic-coverage-measure
  "The semantic-search coverage descriptor, retained so repair can publish a fresh snapshot immediately."
...
(defn- store-repair-metrics!
  [index-id {:keys [orphan-count document-count snapshot-at] :as counts}]
  (let [pgvector        (semantic.env/get-pgvector-datasource!)
        index-metadata  (semantic.env/get-index-metadata)
    ...
    (when index-id
      (jdbc/execute-one!
       ...
       [(format (str "UPDATE %s SET "
                     "repair_orphan_count = ?, "
                     "repair_document_count = ?, "
                     "repair_snapshot_at = ? "
                     "WHERE id = ?")
                metadata-table)
        orphan-count document-count (when counts snapshot-at) index-id])
      (clear-active-index-cache!))))

(defn report-repair-metrics!
  "Publish the repair job's stale-orphan and canonical-document counts through shared index metadata.
  `index-id` pins the snapshot to the index that repair measured; `:snapshot-at` is the time captured before
  reading the canonical document stream. Nil counts invalidate their measures. Reporting failures do not
  fail repair; interruption still propagates."
  ([counts]
   (report-repair-metrics! nil counts))
  ([index-id counts]
   (try
     (store-repair-metrics! index-id counts)
     (search.index-health/refresh-index-check! semantic-coverage-measure)
