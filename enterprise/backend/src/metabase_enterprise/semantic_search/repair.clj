...
(defn count-repair-documents
  "Count the distinct `(model, model_id)` pairs captured from the canonical document stream."
  [pgvector repair-table-name]
  (try
    (or (:n (jdbc/execute-one!
             pgvector
             (-> (sql.helpers/select [:%count.* :n])
    ...
    (catch InterruptedException e
      (throw e))
    (catch Exception e
      (log/errorf e "Error counting documents in repair table %s" repair-table-name)
      nil)))
