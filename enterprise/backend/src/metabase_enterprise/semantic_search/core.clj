...
(defenterprise repair-index!
  ...
  Returns the repaired index ID, its stale-orphan count, distinct canonical document count, and the
  pgvector timestamp captured before reading the canonical stream."
  :feature :semantic-search
    ...
    (if-not active-state
        ...
        (semantic.repair/with-repair-table!
          ...
          (fn [repair-table-name]
            ...
            {:index-id       (-> (semantic.index-metadata/get-active-index-state pgvector index-metadata)
                                 ...
                                 :id)
             :orphans        0
             :document-count (semantic.repair/count-repair-documents pgvector repair-table-name)
      ...
      (semantic.repair/with-repair-table!
        ...
        (fn [repair-table-name]
          ...
          (let [document-count (semantic.repair/count-repair-documents pgvector repair-table-name)
                orphans       (semantic.repair/count-stale-orphans pgvector
