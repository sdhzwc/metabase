...
(deftest semantic-coverage-uses-canonical-repair-count-test
  (let [queries (atom [])]
    (mt/with-dynamic-fn-redefs
      [semantic.health/active-index
       ...
       semantic.health/row-count       (fn [_ sql]
                                         (swap! queries conj sql)
                                         0)]
      (is (=? {:value 0.0
               :health 0
               :message #".*0 of 4 expected items indexed.*"}
              (#'semantic.health/semantic-coverage)))
      (is (= 1 (count @queries)) "coverage reads its denominator from the repair snapshot"))))

(deftest semantic-coverage-without-fresh-repair-count-is-omitted-test
  (doseq [metadata-row [(assoc (:metadata-row active-state)
                               :repair_document_count nil
                               :repair_snapshot_at nil)
                        (assoc (:metadata-row active-state)
                               :repair_document_count 4
                               :repair_snapshot_at
                               (t/minus (t/offset-date-time) (t/hours 4)))]]
    (mt/with-dynamic-fn-redefs
      [semantic.health/active-index (constantly {:pgvector ::pgvector
                                                 :state (assoc active-state :metadata-row metadata-row)})
       semantic.health/row-count    #(throw (ex-info "coverage must not query without a current denominator" {}))]
...
(deftest semantic-staleness-includes-active-dlq-test
  (let [rows    (atom [{:pending 0, :age nil}
                       {:pending 1, :age 25200}])
        queries (atom [])]
    (mt/with-dynamic-fn-redefs
      [semantic.health/active-index
...
(deftest ^:sequential report-repair-metrics!-test
  (mt/with-dynamic-fn-redefs [health-inspector/enabled? (constantly false)]
    (testing "a stored snapshot refreshes both labelled gauges immediately"
      (let [calls  (atom [])
            shared (atom nil)]
        (mt/with-dynamic-fn-redefs
          [semantic.health/store-repair-metrics! (fn [_index-id counts] (reset! shared counts))
           semantic.health/active-index
           #(let [{:keys [orphan-count document-count]} @shared]
              {:pgvector :x
               :state (update active-state :metadata-row assoc
                              :repair_orphan_count orphan-count
                              :repair_document_count document-count
                              :repair_snapshot_at (t/offset-date-time))})
           semantic.env/get-index-metadata       (constantly {:gate-table-name "index_gate"})
           semantic.health/row-count             (constantly 23)
           analytics/set-gauge!                  (fn [& args] (swap! calls conj (vec args)))]
          (semantic.health/report-repair-metrics! {:orphan-count 3, :document-count 23}))
        (is (= #{[:metabase-search/index-coverage-ratio {:index "semantic-search"} 1.0]
                 [:metabase-search/index-garbage-count {:index "semantic-search"} 3]}
               (set @calls)))))
      ...
      (mt/with-dynamic-fn-redefs [semantic.health/store-repair-metrics! (constantly nil)
                                  ...
                                  (constantly {:pgvector :x
                                               :state (update active-state :metadata-row assoc
                                                              ...
                                                              :repair_document_count 23
                                                              :repair_snapshot_at (t/offset-date-time))})
                                  semantic.env/get-index-metadata       (constantly {:gate-table-name "index_gate"})
                                  semantic.health/row-count             (constantly 23)
                                  analytics/set-gauge!                  (fn [& _] (throw (ex-info "boom" {})))]
        (is (nil? (semantic.health/report-repair-metrics! {:orphan-count 3, :document-count 23})))))))

(deftest expired-repair-count-is-omitted-test
  (mt/with-dynamic-fn-redefs
    [semantic.health/active-index
     (constantly {:pgvector :x
                  :state (update active-state :metadata-row assoc
                                 :repair_orphan_count 3
                                 :repair_snapshot_at (t/minus (t/offset-date-time) (t/hours 4)))})]
...
(deftest store-repair-metrics!-writes-exact-index-metadata-test
  (let [calls       (atom [])
    ...
    (mt/with-dynamic-fn-redefs
      ...
      (#'semantic.health/store-repair-metrics! 17 {:orphan-count 7, :document-count 23
                                                   :snapshot-at snapshot-at}))
    (is (= [7 23 snapshot-at 17] (-> @calls first second rest vec)))
    (is (re-find #"repair_orphan_count" (-> @calls first second first)))
    (is (re-find #"repair_document_count" (-> @calls first second first)))))
  ...
  (let [calls       (atom [])
    ...
    (mt/with-dynamic-fn-redefs
      [semantic.env/get-pgvector-datasource!          (constantly ::pgvector)
       ...
       semantic.health/clear-active-index-cache!      (constantly nil)
       jdbc/execute-one!                              (fn [& args] (swap! calls conj args))]
      (#'semantic.health/store-repair-metrics! nil {:orphan-count 7, :document-count 23
                                                    :snapshot-at snapshot-at}))
    (is (= [7 23 snapshot-at 1] (-> @calls first second rest vec)))))
