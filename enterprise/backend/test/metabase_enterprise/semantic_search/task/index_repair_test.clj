  ...
  (let [reported    (atom [])
    ...
    (mt/with-dynamic-fn-redefs
      [search.ingestion/searchable-documents  (constantly [])
       semantic-search.core/repair-index!      (constantly {:index-id 11, :orphans 7, :document-count 23
                                                            :snapshot-at snapshot-at})
       semantic.health/report-repair-metrics! (fn [& args] (swap! reported conj args))]
      (#'index-repair/repair-index!)
      (is (= [[11 {:orphan-count 7, :document-count 23, :snapshot-at snapshot-at}]] @reported)))))
