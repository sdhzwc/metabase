      ...
      (semantic.tu/with-test-db! {:mode :mock-indexed}
        (let [pgvector       (semantic.env/get-pgvector-datasource!)
          ...
          (let [modified-docs [(create-test-document "card" 1 "Dog Training Guide")        ; existing - should remain
                ...
                repair-result (semantic.core/repair-index! modified-docs)]
            (is (= 3 (:document-count repair-result))
                "repair reports the canonical population at the index's deduplication grain")
      ...
      (semantic.tu/with-test-db! {:mode :blank}
        ...
        (with-redefs [semantic.embedding/get-configured-model        (fn [] semantic.tu/mock-embedding-model)
          ...
          (let [pgvector       (semantic.env/get-pgvector-datasource!)
            ...
            (let [result (semantic.core/repair-index! [(create-test-document "card" 1 "Dog Training Guide")])]
              (is (= 1 (:document-count result))))
